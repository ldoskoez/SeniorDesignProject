...
class GestureBlock {
    ...
    gestureCheck({gestureID}) {
        ...
        if(localStorage.getItem('mostRecentCustomGesture') != null)
        {
            var repeat = false;
            //iterate through list and check if the most recent custom gesture has been added yet
            for(var p=0; p<this.menus.KEY.items.length; p++)
            {
                if(this.menus.KEY.items[p].text == localStorage.getItem('mostRecentCustomGesture'))
                {
                    repeat = true;
                }
            ...
            if(repeat == false)
            {
                this.addGestureToMenu(localStorage.getItem('mostRecentCustomGesture'));
            }
